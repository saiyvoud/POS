...
const SideBarComponents = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <aside
      className={
        theme === "light"
          ? `bg-white border-r border-gray-200 min-h-screen ${isOpen ? "w-64" : "w-20"} 
      duration-300 p-5 pt-8 relative flex flex-col transition-all`
          : `bg-slate-900 border-r border-gray-200 min-h-screen ${isOpen ? "w-64" : "w-20"} 
      duration-300 p-5 pt-8 relative flex flex-col transition-all`
      }
    >
      {/* ປຸ່ມ Toggle Sidebar */}
      ...
      <div className="flex gap-x-4 items-center mb-10 px-2">
        ...
        <h1
          className={`text-gray-800 font-bold text-xl duration-200 origin-left ${!isOpen && "scale-0 hidden"}`}
        >
          LaoAdmin
        </h1>
      </div>
          ...
          {menus.map((menu, index) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`font-medium whitespace-nowrap duration-200 ${!isOpen && "opacity-0 hidden"}`}
                >
                  {menu.name}
                </span>
